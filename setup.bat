...
echo %~dp0 | findstr /C:"%TEMP%" >nul && goto :temp

echo "Starting the script... %~f0"
powershell -noprofile "$arg1 = '' + {%1}; $_PSCommandPath = '' + {%~f0}; iex ((Get-Content -LiteralPath $_PSCommandPath) | out-string)" && goto :EOF
...
:temp
echo It looks like you're trying to run it from inside the archive. Make sure you extract the file first.
pause
goto :EOF

:badpath
echo You put the Unlocker in a path that will break the setup script. Move it somewhere else, for example "C:\unlocker" or "D:\unlocker".
pause
goto :EOF
...
function Common-Setup-Real {
    ...
    Catch {
        ...
        Else {
            Warn 'Permission error, requesting administrator rights...'
            $process = Start-Process -FilePath $_PSCommandPath -Verb RunAs -WorkingDirectory . -ArgumentList $action -PassThru -Wait
